...
print("cuda from torch " + str(torch.cuda.is_available()))
print("cuda from numba ")
print(cuda.detect())
...
def train(config_path,
    ...
    if multi_gpu:
        gpu_list = []
        print(type(num_gpus))
        for i in range(num_gpus or torch.cuda.device_count()):
            gpu_list.append(i)
    ...
    if multi_gpu:
        num_gpu = num_gpus or torch.cuda.device_count()
        ...
        while True:
            ...
            for example in dataloader:
                ...
                if global_step % steps_per_eval == 0:
                    if train_cfg.enable_mixed_precision:
                        torchplus.train.save_models(model_dir, [net, amp_optimizer, { "amp_optimizer": scaler }],
                                                net.get_global_step())
                    else:
                        torchplus.train.save_models(model_dir, [net, amp_optimizer],
                                                net.get_global_step())
                    if not eval_diff_device:
                        net.eval()
                        ...
                        model_logging.log_text("#################################",
                                            global_step)
                        detections, result_dict = evaluate(config_path, model_dir, device_name=eval_diff_device)
                        for k, v in result_dict["results"].items():
                            model_logging.log_text("Evaluation {}".format(k), global_step)
                            model_logging.log_text(v, global_step)
                        model_logging.log_metrics(result_dict["detail"], global_step)
    ...
    #                             net.get_global_step())
    if train_cfg.enable_mixed_precision:
        torchplus.train.save_models(model_dir, [net, amp_optimizer, scaler],
                                step)
    else:
        torchplus.train.save_models(model_dir, [net, amp_optimizer],
                                step)
